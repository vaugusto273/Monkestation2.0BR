...
/obj/item/storage/part_replacer/bluespace/tier4/bst
	...
	desc = "A specialized bluespace RPED for technicians that can manufacture stock parts on the fly. Alt-Right-Click to manufacture parts."
...
/obj/item/storage/part_replacer/bluespace/tier4/bst/alt_click_secondary(mob/user)
	...
	var/spawn_selection = tgui_input_list(user, "Pick a part, or clear storage", "RPED Manufacture", list("Cables", "Glass", "Spare T4s", "Machine Board", "Stock Part", "Beaker"))
	...
	if(isnull(spawn_selection))
		...
		for(var/i in 1 to 10)
			...
			atom_storage.attempt_insert(new /obj/item/stock_parts/manipulator/femto(src), user, TRUE)
