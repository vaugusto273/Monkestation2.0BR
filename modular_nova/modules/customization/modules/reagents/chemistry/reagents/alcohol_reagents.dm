...
/* ERROR ON PORTING FROM NOVA SECTOR
/datum/reagent/consumable/ethanol/hotlime_miami/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(affected_mob.stamina.loss(-2 * REM * seconds_per_tick, updating_stamina = FALSE))
...
/* no snail AAAA GAIJIN LEAVE ME ALONE
...
/* no luminescent on monkey NO LED GUYS
...
/* no feline on monkey :pray: :pray: :pray:
...
/* ERROR ON PORTING NOVA HERE for some reason
...
/datum/reagent/consumable/ethanol/hippie_hooch/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	affected_mob.reagents.remove_reagent(/datum/reagent/consumable/ethanol, 3 * REM * seconds_per_tick)
	. = ..()
...
/datum/reagent/consumable/ethanol/cringe_weaver/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	. = ..()
	var/obj/item/organ/internal/liver = drinker.get_organ_slot(ORGAN_SLOT_LIVER)
