...
/datum/element/tumor_corruption/proc/handle_organ_corruption_on_existing_organs(mob/living/carbon/organ_enjoyer)
	var/obj/item/organ/internal/liver = organ_enjoyer.get_organ_slot(ORGAN_SLOT_LIVER)
	...
	var/obj/item/organ/internal/stomach = organ_enjoyer.get_organ_slot(ORGAN_SLOT_STOMACH)
	if(stomach && !(stomach.organ_flags & ORGAN_TUMOR_CORRUPTED))
		stomach.AddComponent(/datum/component/organ_corruption/stomach)

	var/obj/item/organ/internal/tongue = organ_enjoyer.get_organ_slot(ORGAN_SLOT_TONGUE)
